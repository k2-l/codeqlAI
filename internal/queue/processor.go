...
func (p *Processor) HandleCodeQLScan(ctx context.Context, t *asynq.Task) error {
        ...
        if err := p.db.First(&existingTask, "id = ?", taskID).Error; err != nil {
                log.Warn("task no longer exists in database, skipping")
                return nil // 返回 nil 不触发重试
        }

        // 确保 storage 目录存在
        storageDir := filepath.Join("storage", payload.TaskID)
        if err := os.MkdirAll(storageDir, 0755); err != nil {
                return p.failTask(taskID, "failed to create storage dir: %w", err)
        }
        ...
        if payload.IsGitSource() {
                ...
                cloneDir := filepath.Join(storageDir, "source")
                ...
                if err != nil {
                        return p.failTask(taskID, "git clone failed: %w", err)
                }
                sourceRoot = cloneDir
        ...
        if err := p.executor.CreateDatabase(payload.TaskID, payload.Language, sourceRoot); err != nil {
                return p.failTask(taskID, "CreateDatabase failed: %w", err)
        }
        p.updateTaskField(taskID, "codeql_db_path", p.executor.DBPath(payload.TaskID))
        ...
        if payload.CustomRuleID != "" {
                ...
                if err != nil {
                        return p.failTask(taskID, "invalid custom_rule_id: %w", err)
                }
                customQLPath, err = p.ruleService.GetRuleFilePath(ruleID)
                if err != nil {
                        return p.failTask(taskID, "failed to get custom rule file: %w", err)
                }
        }

        if err := p.executor.RunAnalysis(payload.TaskID, payload.Language, customQLPath); err != nil {
                return p.failTask(taskID, "RunAnalysis failed: %w", err)
        }
        sarifPath := p.executor.SarifPath(payload.TaskID)
        p.updateTaskField(taskID, "sarif_path", sarifPath)
        ...
        if err != nil {
                return p.failTask(taskID, "ParseSarif failed: %w", err)
        }

        if len(findings) > 0 {
                if err := p.db.Create(&findings).Error; err != nil {
                        return p.failTask(taskID, "failed to save findings: %w", err)
                }
        }
...
func (p *Processor) updateTaskStatus(taskID uuid.UUID, status model.TaskStatus) {
        p.db.Model(&model.Task{}).Where("id = ?", taskID).
                Update("status", status)
}

// updateTaskField 更新任务单个字段
func (p *Processor) updateTaskField(taskID uuid.UUID, field string, value interface{}) {
        p.db.Model(&model.Task{}).Where("id = ?", taskID).
                Update(field, value)
}

// failTask 标记任务失败并返回错误
func (p *Processor) failTask(taskID uuid.UUID, format string, args ...interface{}) error {
