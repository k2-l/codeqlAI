...
// timeFormat 任务名时间格式
const taskNameTimeFormat = "20060102-150405"
...
func (s *ScanService) SubmitScan(req SubmitScanRequest) (*model.Task, error) {
	...
	if displayName == "" {
		displayName = fmt.Sprintf("%s-%s-%s", req.ProjectName, req.Language, time.Now().Format(taskNameTimeFormat))
	}
...
func (s *ScanService) DeleteTask(taskID uuid.UUID) error {
	...
	err := s.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&model.Finding{}).Where("task_id = ?", taskID).Pluck("id", &findingIDs).Error; err != nil {
			return err
		}

		// 删除 ai_results
		if len(findingIDs) > 0 {
			if err := tx.Where("finding_id IN ?", findingIDs).Delete(&model.AiResult{}).Error; err != nil {
				return err
			}
		}

		// 删除 findings
		if err := tx.Where("task_id = ?", taskID).Delete(&model.Finding{}).Error; err != nil {
			return err
		}
		...
		var taskCount int64
		if err := tx.Model(&model.Task{}).Where("project_id = ?", projectID).Count(&taskCount).Error; err != nil {
			return err
		}
		if taskCount == 0 {
			if err := tx.Delete(&model.Project{}, "id = ?", projectID).Error; err != nil {
				return err
			}
...
// cancelPendingTasks 取消队列中待执行的任务
func (s *ScanService) cancelPendingTasks(taskID uuid.UUID) {
	pendingTasks, err := s.inspector.ListPendingTasks("default", asynq.PageSize(100))
	if err != nil {
		logger.Warn("failed to list pending tasks", zap.Error(err))
		return
	}
	...
	for _, t := range pendingTasks {
		...
		if strings.Contains(string(t.Payload), taskIDStr) {
